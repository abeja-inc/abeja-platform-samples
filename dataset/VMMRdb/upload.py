...
def build_props(image_root_dir, min_num_image_per_class):
    image_list = []
    for p in image_root_dir.iterdir():
        num_images = len(list(p.iterdir()))
        label = p.name
        if min_num_image_per_class >= num_images:
            print(f'skip label {label}, num_images: {num_images}')
            continue
        image_list.append(label)

    labels = [dict(label_id=i, label=name)
...
def upload_images(channel, dataset, image_root_dir: Path, label2id: Dict[str, int], min_num_image_per_class, max_workers):
    ...
    with Executer(max_workers=max_workers) as executer:
        ...
        for image_dir in tqdm(image_root_dir.iterdir(), total=total, position=0):
            ...
            if min_num_image_per_class >= num_images:
                print(f'skip {label}, num_images: {num_images}')
                continue
            future_pool += list(tqdm(executer.map(_f,
                                                  image_dir.iterdir()), total=num_images, position=1))
    return future_pool


def main(image_root_dir, channel_id, dataset_name, max_workers, min_num_image_per_class):
    ...
    dataset = datasets_client.datasets.create(
        name=dataset_name, type='classification', props=props)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--channel_id', help='channel_id to upload images', type=str)
    parser.add_argument(
        '--dataset_name', help='dataset name to be created', default='VMMRdb', type=str)
    parser.add_argument('--max_workers', default=1, type=int)
    parser.add_argument('--min_num_image_per_class', default=1, type=int)
    args = parser.parse_args()
    main(args.image_root_dir, args.channel_id,
         args.dataset_name, args.max_workers, args.min_num_image_per_class)
